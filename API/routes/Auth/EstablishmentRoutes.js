...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const phoneExists = await Establishment.findOne({
      ...
      const createdEstablishment = await Establishment.findById(
        newEstablishment._id
      )
        .select("-password")
        .select("-__v")
        .populate("details");
      ...
      return res
        .status(201)
        .json({ establishment: createdEstablishment, token });
    } catch (error) {
...
router.post("/login-establishment", async (req, res) => {
  try {
    ...
    if (!establishment) {
      return res
        .status(404)
        .json({ error: "No establishment found with this email!" });
    }
...
router.get("/fetch-establishment-type", async (req, res) => {
  try {
    ...
    if (establishments.docs.length === 0) {
      return res
        .status(404)
        .send("Establishments not found for the specified company type");
    }
...
router.get("/get-details/:establishmentsId", async (req, res) => {
  try {
    ...
    switch (typeEstablishment) {
      case "promoter":
        establishmentDetails = await PromoterDetails.findOne({
          establishmentId: establishmentsId,
        });
      case "bar":
        establishmentDetails = await BarDetails.findOne({
          establishmentId: establishmentsId,
        });
        break;
      case "club":
        establishmentDetails = await ClubDetails.findOne({
          establishmentId: establishmentsId,
        });
        break;
      case "kiosk":
        establishmentDetails = await KioskDetails.findOne({
          establishmentId: establishmentsId,
        });
        break;
      default:
...
router.get("/fetchEventIsFeatured/:isFeatured", async (req, res) => {
  try {
    ...
    if (events.length === 0) {
      return res
        .status(404)
        .json({ error: `No Featured establishments so far` });
    }
...
router.delete("/delete/:establishmentId", checkToken, async (req, res) => {
  try {
    const establishmentId = req.params.establishmentId;
