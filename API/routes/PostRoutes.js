...
router.post("/create-post", muiltiImagesMiddleware.array('images'), async (req, res) => {
  try {
    const files = req.files; // Imagens enviadas na solicitação
    const { establishmentObjId, content, eventType, products, tags, location, expirationDate, eventStartTime, eventEndTime, isRecurring } = req.body; // Dados da postagem enviados no corpo da solicitação
